...
class TrackingProcessor:
    ...
    def _create_stay_suggestion(cls, trip, day_id, points, explicit_end_time=None):
        ...
        TrackingSuggestion.objects.create(
            ...
            lat=avg_lat,
            lon=avg_lon,
            notes=f"Automatisch erkannter Aufenthalt in {place_name if place_name else 'diesem Ort'} von {first.timestamp_local.strftime('%H:%M')} bis {end_time.strftime('%H:%M')}."
        )
    ...
    def _create_transport_suggestion(cls, trip, day_id, points):
        ...
        if dest_name:
            title = f"Fahrt nach {dest_name.split(',')[0]}"
        else:
        ...
        TrackingSuggestion.objects.create(
            ...
            lat=last.lat, 
            lon=last.lon,
            notes=f"Zurückgelegte Distanz: ca. {dist_km:.1f} km in {duration_mins} Minuten nach {dest_name if dest_name else 'Zielort'}."
        )
