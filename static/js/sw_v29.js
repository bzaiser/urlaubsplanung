const CACHE_NAME = 'travel-hub-v29';
const STATIC_CACHE = 'travel-hub-static-v29';
const MEDIA_CACHE = 'travel-hub-media-v5';
const DYNAMIC_CACHE = 'travel-hub-dynamic-v5';

const log = (msg, data = '') => console.log(`[SW v29] ${msg}`, data);
...
<body>
    <div class="container">
        <div class="indicator">OFFLINE-MODUS (v29)</div>
...
self.addEventListener('install', (event) => {
    log('Installing Software v29 (Cache-Bust Fix)...');
    self.skipWaiting();
...
self.addEventListener('activate', (event) => {
    log('v29 Activated. Clearing old caches...');
...
self.addEventListener('fetch', (event) => {
    ...
    // FIX: Never cache edit/delete routes - ALWAYS go to network
    ...
    event.respondWith(
        caches.match(event.request).then((cachedResponse) => {
            // v29: Network-First for dynamic routes, Cache-First only for known static assets
