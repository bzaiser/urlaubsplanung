/**
 * Offline Manager for Travel Hub (v29)
 * Clean, safe implementation for background sync and offline storage.
 * Wrapped in block scope to prevent "Identifier already declared" errors.
 */

{
    ...
    async function resetPWAData() {
        if (!confirm("⚠️ Alle lokalen Daten und Cache löschen?")) return;
        try {
            if ('caches' in window) {
    ...
    window.resetPWAData = resetPWAData;
